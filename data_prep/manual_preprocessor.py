...
def load_preprocess_function(preprocess_type):
    print("Manual preprocess type: ", preprocess_type)
    if preprocess_type is None:
        return lambda x:x
    elif preprocess_type == "v1":
        return manual_processor_v1
    elif preprocess_type == "v2":
        return manual_processor_v2    
    elif preprocess_type == "v3":
        return manual_processor_v3
    elif preprocess_type == "v4":
        return manual_processor_v4
    elif preprocess_type == "v5":
        return manual_processor_v5
    else:
        raise
...
lac_b, lac_u = 0, 25
mag_b, mag_u = 0, 5
phos_b, phos_u = 0, 20 # Need to check conversion from MIMIC -> Fixed(190802)
bi_t_b, bi_t_u = 0, 40
hct_b, hct_u = 0, 50
...
def process_func5(ele):
    with np.errstate(invalid='ignore'):
        # HR
        nan_idx = np.isnan(ele[:, 0])
        ele[:, 0] = (ele[:, 0] > 90).astype(float)
        ele[:, 0][nan_idx] = np.nan
        
        # Temp
        nan_idx = np.isnan(ele[:, 2])
        temp1 = (ele[:, 2] < 36).astype(float)
        temp2 = (ele[:, 2] > 38).astype(float)
        ele[:, 2] = ((temp1 + temp2) > 0).astype(float)
        ele[:, 2][nan_idx] = np.nan

        # Resp/PaCO2
        nan_idx1 = np.isnan(ele[:, 6])# Resp
        nan_idx2 = np.isnan(ele[:, 12])# PaCO2
        resp = (ele[:, 6] > 20).astype(float)
        paco = (ele[:, 12] < 32).astype(float)
        anom = ((resp + paco) > 0).astype(float)
        nan_idx = nan_idx1 * nan_idx2
        ele[:, 6] = anom
        ele[:, 12] = anom
        ele[:, 6][nan_idx] = np.nan
        ele[:, 12][nan_idx] = np.nan

        # WBC
        nan_idx = np.isnan(ele[:, 31])
        wbc1 = (ele[:, 31] < 4).astype(float)
        wbc2 = (ele[:, 31] > 12).astype(float)
        ele[:, 31] = ((wbc1 + wbc2) > 0).astype(float)
        ele[:, 31][nan_idx] = np.nan
    
    return ele

def manual_processor_v1(data):
    newdata = []
    print("Manual preprocessing ...")
    for ele in tqdm(data):
        ele = process_func1(ele)
        ele = process_func2(ele)
        newdata.append(ele)
    return newdata

def manual_processor_v2(data):
    newdata = []
    print("Manual preprocessing ...")
    for ele in tqdm(data):
        ele = process_func1(ele)
...
def manual_processor_v3(data):
    newdata = []
    print("Manual preprocessing ...")
    for ele in tqdm(data):
        ele = process_func1(ele)
        ele = process_func2(ele)
        ele = process_func3(ele)
        newdata.append(ele)
    return newdata
...
def manual_processor_v4(data):
    ...
    for ele in tqdm(data):
        ele = process_func1(ele)
        ele = process_func4(ele)        
        ele = process_func2(ele)
        ele = process_func3(ele)
        newdata.append(ele)
    return newdata

def manual_processor_v5(data):
    newdata = []
    print("Manual preprocessing ...")
    for ele in tqdm(data):
        ele = process_func1(ele)
        ele = process_func4(ele)
        ele = process_func5(ele)        
        ele = process_func2(ele)
        ele = process_func3(ele)
        newdata.append(ele)
    return newdata
